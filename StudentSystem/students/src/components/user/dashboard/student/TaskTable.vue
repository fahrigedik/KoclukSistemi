...
const getStatusColor = (status) => {
    switch (status) {
        case 'Tamamlanmadı': return 'grey'
        case 'Devam Ediyor': return 'blue'
...
const toggleComplete = async (task) => {
    ...
    try {
        task.updating = true
        await userTaskService.toggleTaskComplete(task.id)
        await fetchTasks()
...
const toggleWorking = async (task) => {
    const previousState = task.isWorking
    try {
        task.updatingWork = true
        await userTaskService.toggleTaskWorking(task.id)
        await fetchTasks()
        toast.success('Çalışma durumu güncellendi')
    } catch (err) {
        task.isWorking = previousState
        toast.error('Güncelleme başarısız')
        console.error('Update error:', err)
    } finally {
        task.updatingWork = false
    }
}

const fetchTasks = async () => {
    try {
        const response = await userTaskService.getTasks()
